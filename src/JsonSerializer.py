...
def load_from_json(obj, json_str, encoding='ascii'):
    ...
    return load_from_json_object(obj, json_object)


def load_from_json_object(obj, json_object):
    if isinstance(json_object, dict):
        return load_from_json_dict(obj, json_object)
    elif isinstance(json_object, list):
        return load_from_json_array(obj, json_object)
    else:
        return json_object


def load_from_json_dict(obj, json_dict):
    for (k, v) in json_dict.items():
        if not hasattr(obj, k):
            raise ValueError("obj[%s] has no attribute[%s]" % (str(type(obj)), k))
        subobj = getattr(obj, k)
        subobj = load_from_json_object(subobj, v)
        setattr(obj, k, subobj)
    return obj


def load_from_json_array(array_obj, json_array):
    if isinstance(array_obj, FixedTypeList):
        element_type = array_obj.element_type
        for json_element in json_array:
            element_obj = load_from_json_object(element_type(), json_element)
...
def dump_to_json(obj):
    json_object = dump_to_json_object(obj)
    return json.dumps(json_object)


def dump_to_json_object(obj):
    if isinstance(obj, list):
        return dump_json_array(obj)
    elif is_native_json_type(obj):
        return obj
    else:
...
def dump_json_array(obj):
    sub_obj_list = []
    for s in obj:
        sub_obj_list.append(dump_to_json_object(s))
    return sub_obj_list
...
def dump_custom_object(obj):
    ...
    for attr_name in attr_list:
        ...
        json_object[attr_name] = dump_to_json_object(subobj)
    return json_object
